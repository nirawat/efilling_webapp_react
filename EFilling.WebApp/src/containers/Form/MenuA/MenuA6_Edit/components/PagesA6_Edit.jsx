...
class PagesForm extends PureComponent {
  ...
  componentDidMount() {
    ...
    Axios
      .get(`PublicDocMenuA/MenuA6EditInterfaceData/${eFillingSys.registerId}/${urlParams.get('id')}`)
      .then((resp) => {
        ...
        this.setState({
          ...
          renewRound: resp.data.editdata.renewround,
  ...
  handleSubmit = (e) => {
    ...
    Axios
      .post('/PublicDocMenuA/UpdateDocMenuA6Edit', this.state)
      .then((resp) => {
  ...
  handlePrintReport = () => {
    ...
    Axios
      .get(`PublicDocMenuReport/GetReportR7/${docId}`)
      .then((resp) => {
        if (resp.data !== null) {
          const url = resp.data.filebase64;
          const a = document.createElement('a');
  ...
  handleFileDownloadId = (e) => {
    const { docId } = this.state;
    Axios
      .get(`PublicDocMenuA/GetA6DownloadFileById/${docId}/${e}`)
      .then((resp) => {
