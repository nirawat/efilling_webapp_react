...
class PagesForm extends PureComponent {
  ...
  componentDidMount() {
    ...
    Axios
      .get(`PublicDocMenuA/MenuA5EditInterfaceData/${eFillingSys.registerId}/${urlParams.get('id')}`)
      .then((resp) => {
  ...
  handleChangePosition = (e) => {
    this.setState({ positionNameThai: e.value });
  }
  ...
  handlePrintReport = () => {
    ...
    Axios
      .get(`PublicDocMenuReport/GetReportR3/${docId}`)
      .then((resp) => {
        if (resp.data !== null) {
          const url = resp.data.filebase64;
          const a = document.createElement('a');
